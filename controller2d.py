...
class Controller2D(object):
    ...
    def calculate_throttle(self, t, v, v_desired):
        ...
        i_term = self._kI * time_step * speed_error + self.vars.i_term_previous
        self.vars.i_term_previous = i_term
        return k_term + i_term

    # mostly for lateral control
    ...
    def get_steering_direction(self, v1, v2):
        cross_prod = v1[0] * v2[0] - v1[1] * v2[1]
        if cross_prod >= 0:
            return -1
        return 1
    ...
    def calculate_steering(self, x, y, yaw, waypoints, v):
        ...
        steering = self.get_steering_direction(v1, v2) * np.arctan(
            (2 * self._wheelbase * np.sin(alpha)) / self.get_lookahead_dis(v)
        )
